...
read_data<-function() {
  tbl<-read.csv("C:/Users/narjunan/Documents/02RPR/activity.csv",header=T,colClasses=c("numeric", "character", "numeric"))
...
library(ggplot2)
ggplot(steps_per_day_data,aes(x=steps))+
...
ggplot(avg_steps_data,aes(x=as.numeric(interval),y=steps))+
  geom_line(color="blue",size=1)+
  labs(title="Average daily activity pattern", x="Interval", y="Number of steps") 
   max_steps<-avg_steps_data[avg_steps_data[,2]==max(avg_steps_data$steps),1]
```
Maximum number of steps is `r max_steps ` 
...
complete_tbl <- data.frame(  
    steps = impute_mean(tbl),  
    date = tbl$date,  
    interval = tbl$interval)
```
The number of rows with missing value `r nrow(tbl[!complete.cases(tbl),]) `

```{r}
steps_per_day_data1 <- steps_per_day_fn(complete_tbl)
#Load ggplot2 function 
library(ggplot2)
ggplot(steps_per_day_data1,aes(x=steps))+
  geom_histogram(fill="blue",binwidth=1000)+
  labs(title="Histogram of Steps Taken per Day exluding missing values", x="Number of Steps", y="Count") 
